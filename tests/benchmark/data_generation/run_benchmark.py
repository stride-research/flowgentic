...
class FlowGenticBenchmark:
	...
	async def run_workloads(
		...
		"""
		Run workloads with SHARED backend across all agents.
		This properly tests parallelization of flowgentic setup work.
		"""
		...
		(
			...
			execution_time,
			results,
		) = await workload_manager.get_shared_backend_workload(id="langgraph_asyncflow")

		t_end = time.perf_counter()
		makespan = t_end - t_start

		logger.debug(
			f"WORKLOAD - MAKESPAN: {makespan} \
				...
				SETUP OVERHEAD (wall-clock): {flowgentic_overhead}\
				"
		)
		return makespan, flowgentic_overhead, execution_time
	...
	async def run_strong_scaling(self, config: BenchmarkConfig):
		...
		for backend_slots in backend_slots_options:
			...
			result = BenchmarkResult(
				...
				tool_execution_duration_time=config.tool_execution_duration_time,
				makespan=makespan,
				total_flowgentic_overhead=flowgentic_overhead_wallclock,
