...
def test_short_term_memory_item():
	...
	assert (
		abs(item.importance - 2.0) < 0.1
	)  # System messages have higher importance (with length bonus)
	...
	assert (
		abs(item.importance - 1.2) < 0.1
	)  # Human messages have moderate importance (with length bonus)
