...
class ShortTermMemoryItem(BaseModel):
    ...
    def _calculate_importance(message: BaseMessage) -> float:
        ...
        content_length = len(message.content) if isinstance(message.content, str) else len(str(message.content))
...
class ShortTermMemoryManager:
    ...
    def _apply_trimming_strategy(self) -> List[BaseMessage]:
        ...
        elif self.config.short_term_strategy == "summarize" and self.llm and self.config.enable_summarization:
    ...
    def _summarize_old_messages(self) -> List[BaseMessage]:
        ...
        # Always keep system messages
        system_msgs = cast(List[BaseMessage], [m for m in self.message_history if isinstance(m, SystemMessage)])
        other_msgs = [m for m in self.message_history if not isinstance(m, SystemMessage)]
        ...
        recent_msgs = other_msgs[-keep_unsummarized:] if keep_unsummarized < len(other_msgs) else other_msgs

        # Messages to summarize
        messages_to_summarize = other_msgs[:-keep_unsummarized] if keep_unsummarized < len(other_msgs) else []
    ...
    def _create_conversation_summary(self, messages: List[BaseMessage]) -> Optional[BaseMessage]:
...
Summary:"""
        ...
        try:
            ...
            if hasattr(response, 'content') and response.content:
                ...
                summary_content = f"Previous conversation summary: {content_str.strip()}"
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_messages": len([m for m in self.message_history if isinstance(m, SystemMessage)]),
            "human_messages": len([m for m in self.message_history if isinstance(m, HumanMessage)]),
            "ai_messages": len([m for m in self.message_history if isinstance(m, AIMessage)]),
        }
    ...
    def consolidate_memory(self) -> Dict[str, Any]:
        ...
        if len(self.message_history) != len(self.memory_items):
            ...
            self.memory_items = [ShortTermMemoryItem.from_message(msg) for msg in self.message_history]
        ...
        return {
            ...
            "avg_importance": sum(item.importance for item in self.memory_items) / len(self.memory_items) if self.memory_items else 0.0
        }
    ...
    def _cleanup_low_importance_items(self):
        ...
        for i, (msg, item) in enumerate(zip(self.message_history, self.memory_items)):
            if isinstance(msg, SystemMessage) or item.importance >= min_importance_threshold:
...
class MemoryManager:
    ...
    async def get_relevant_context(
        ...
        context = {
            ...
            "memory_stats": self.short_term_manager.get_memory_stats(),
            "relevant_messages": self._get_relevant_messages(short_term_messages, query),
        }
    ...
    def _get_relevant_messages(
        ...
        for i, msg in enumerate(messages):
            ...
            importance_boost = self.short_term_manager.memory_items[i].importance if i < len(self.short_term_manager.memory_items) else 1.0
            recency_boost = max(0.5, 1.0 - (len(messages) - i) / len(messages))  # More recent = higher boost
    ...
    def _calculate_semantic_relevance(self, content: str, query: str) -> float:
        ...
        for i in range(len(query_tokens)):
            for j in range(len(content_tokens) - i):
                if query_tokens[i:i+2] == content_tokens[j:j+2]:  # Bigram matching
                    consecutive_boost += 0.2
                elif i < len(query_tokens) - 2 and query_tokens[i:i+3] == content_tokens[j:j+3]:  # Trigram matching
    ...
    def get_memory_health(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            "memory_efficiency": len(self.short_term_manager.message_history) / self.config.max_short_term_messages if self.config.max_short_term_messages > 0 else 1.0,
            "average_importance": sum(item.importance for item in self.short_term_manager.memory_items) / len(self.short_term_manager.memory_items) if self.short_term_manager.memory_items else 0.0,
        })
        return stats
