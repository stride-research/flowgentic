...
try:
	...
	logger.warning(
		"Dragon DDict not available - distributed memory will fall back to in-memory storage"
	)
	DDict = None
		...
		**kwargs,
		...
		if DRAGON_AVAILABLE and DDict is not None:
			try:
				...
				logger.info(
					f"Initialized DragonDDictStore with {n_nodes} nodes, {total_mem / (1024**3):.2f} GB total memory"
				)
			except Exception as e:
				logger.warning(
					f"Failed to initialize Dragon DDict: {e}. Falling back to in-memory storage."
				)
				self.ddict = {}
		...
		**kwargs,
		...
		if DRAGON_AVAILABLE and DDict is not None:
			try:
				...
				self.using_dragon = True
				logger.info(
					f"Initialized DragonCheckpointSaver with {n_nodes} nodes, {total_mem / (1024**3):.2f} GB total memory"
				)
			except Exception as e:
				logger.warning(
					f"Failed to initialize Dragon DDict for checkpoints: {e}. Falling back to in-memory storage."
				)
				self.checkpoints = {}
				self.writes = {}
				self.using_dragon = False
			...
			logger.info(
				"Dragon not available - using in-memory dictionary for checkpoints"
			)
			self.checkpoints = {}
			self.writes = {}
			self.using_dragon = False
