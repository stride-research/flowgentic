...
async def weather_tool(
	city: str, context: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
...
async def demonstrate_memory_with_tools():
	...
	for i, user_message in enumerate(conversation, 1):
		...
		await memory_manager.add_interaction(
			"demo_user", [HumanMessage(content=user_message)]
		)
		...
		elif "calculate" in user_message.lower() or any(
			op in user_message for op in ["*", "+", "-", "/"]
		):
			tool_name = "calculate"
			# Extract expression (simplified)
			tool_args = {
				"expression": user_message.split("calculate", 1)[-1].strip()
		...
		if tool_name:
			...
			if tool_name == "get_weather":
				...
				result = {
					...
					"memory_messages": memory_stats["total_messages"],
					"relevant_found": True,
				}
		...
		print(
			f"After tool execution: {updated_stats['total_messages']} messages in memory"
		)
	...
	for query in test_queries:
		...
		print(
			f"Query '{query}': found {relevant_count} relevant messages from {len(context.get('short_term', []))} total"
		)
...
async def demonstrate_memory_consolidation():
	...
	for i in range(15):
		...
		if (i + 1) % 5 == 0:
			...
			print(
				f"After {i + 1} interactions: {stats['total_messages']} messages (consolidated)"
			)
