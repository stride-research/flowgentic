...
async def demonstrate_memory_strategies():
	...
	messages = cast(
		...
		[
			...
			HumanMessage(
				content="Tell me about memory management"
			),  # This will trigger trimming
	...
	for i, item in enumerate(manager.memory_items[-5:]):  # Show last 5
		print(
			f"  {i + 1}. {item.message_type}: {item.importance:.2f} - {item.content[:40]}..."
		)
	...
	print(
		f"  Configuration: max_messages={health['config']['max_short_term_messages']}, strategy={health['config']['short_term_strategy']}"
	)
...
async def interactive_memory_demo():
	...
	while True:
		...
		ai_response = AIMessage(
			content=f"I received your message: '{user_input}' (with {len(context.get('short_term', []))} messages in context)"
		)
