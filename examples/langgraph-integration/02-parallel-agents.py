...
async def main():
	...
	@integration.asyncflow_tool(
		retry=RetryConfig(
			max_attempts=3, base_backoff_sec=0.5, max_backoff_sec=6.0, timeout_sec=15.0
		)
	)
	...
	@integration.asyncflow_tool(
		retry=RetryConfig(
			max_attempts=3, base_backoff_sec=0.5, max_backoff_sec=6.0, timeout_sec=20.0
		)
	)
	...
	@integration.asyncflow_tool(
		retry=RetryConfig(
			max_attempts=3, base_backoff_sec=0.4, max_backoff_sec=6.0, timeout_sec=12.0
		)
	)
	...
	@integration.asyncflow_tool(
		retry=RetryConfig(
			max_attempts=2, base_backoff_sec=0.5, max_backoff_sec=4.0, timeout_sec=10.0
		)
	)
	...
	async def weather_agent(task: str, location: str) -> Dict[str, Any]:
		...
		messages = [
			SystemMessage(
				content="You are a weather specialist. Provide comprehensive weather analysis."
			),
			HumanMessage(
				content=f"Analyze weather conditions for {location}. Task: {task}"
			),
		]
		...
		if hasattr(response, "tool_calls") and response.tool_calls:
			...
			for tool_call in response.tool_calls:
				if tool_call["name"] == "get_detailed_weather":
					result = await get_detailed_weather.ainvoke(
						{"city": tool_call["args"]["city"]}
					)
					tool_results.append(result)
				elif tool_call["name"] == "get_weather_alerts":
					result = await get_weather_alerts.ainvoke(
						{"city": tool_call["args"]["city"]}
					)
					tool_results.append(result)
			...
			final_messages = messages + [
				response,
				HumanMessage(
					content=f"Tool results: {tool_results}. Provide final weather analysis."
				),
			]
		...
		return {"agent": "weather", "result": response.content, "tools_used": []}
	...
	async def travel_agent(task: str, origin: str, destination: str) -> Dict[str, Any]:
		...
		messages = [
			SystemMessage(
				content="You are a travel specialist. Help with flights, documentation, and travel logistics."
			),
			HumanMessage(
				content=f"Help plan travel from {origin} to {destination}. Task: {task}"
			),
		]
		...
		if hasattr(response, "tool_calls") and response.tool_calls:
			...
			for tool_call in response.tool_calls:
				if tool_call["name"] == "find_flights":
					...
					result = await get_travel_requirements.ainvoke(
						{"destination": tool_call["args"]["destination"]}
					)
					tool_results.append(result)
			...
			final_messages = messages + [
				response,
				HumanMessage(
					content=f"Tool results: {tool_results}. Provide final travel recommendations."
				),
			]
			final_response = await llm.ainvoke(final_messages)
		...
		return {"agent": "travel", "result": response.content, "tools_used": []}
	...
	async def local_agent(task: str, city: str) -> Dict[str, Any]:
		...
		messages = [
			SystemMessage(
				content="You are a local guide specialist. Provide insights about attractions, culture, and local tips."
			),
		...
		if hasattr(response, "tool_calls") and response.tool_calls:
			...
			for tool_call in response.tool_calls:
				if tool_call["name"] == "find_attractions":
					result = await find_attractions.ainvoke(
						{"city": tool_call["args"]["city"]}
					)
					tool_results.append(result)
				elif tool_call["name"] == "get_local_tips":
					result = await get_local_tips.ainvoke(
						{"city": tool_call["args"]["city"]}
					)
					tool_results.append(result)
			...
			final_messages = messages + [
				response,
				HumanMessage(
					content=f"Tool results: {tool_results}. Provide final local recommendations."
				),
			]
			final_response = await llm.ainvoke(final_messages)
		...
		return {"agent": "local", "result": response.content, "tools_used": []}
	...
	async def parallel_agent_workflow(origin: str, destination: str) -> Dict[str, Any]:
		...
		weather_future = weather_agent(
			task="Analyze weather conditions and provide forecast", location=destination
		)
		...
		local_future = local_agent(
			task="Recommend attractions and provide local tips", city=destination
		)
	...
	async def coordinator_node(state: MultiAgentState):
		...
		if "paris" in last_message.lower() and "london" in last_message.lower():
			logger.info(
				"[Coordinator] Dispatching parallel agents for Paris->London trip..."
			)
			...
			final_response = await coordinator_llm.ainvoke(
				[HumanMessage(content=synthesis_prompt)]
			)

			return {"messages": [final_response], "agent_results": agent_results}
	...
	if agent_results:
		...
		for agent_name, result in agent_results.items():
			logger.info(
				f"  {agent_name.upper()} Agent: Used tools {result.get('tools_used', [])}"
			)
