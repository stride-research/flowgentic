...
async def main_with_monitoring() -> int:
	...
	async with AcademyIntegration(flow) as integration:
		...
		assert last_count > first_count, (
			f"Counter should have increased: {first_count} -> {last_count}"
		)
		logger.info(
			f"Counter increased from {first_count} to {last_count} over monitoring period"
		)
...
async def main_parallel_monitoring() -> int:
	...
	async with AcademyIntegration(flow) as integration:
		...
		async def monitor_counter(counter_id: int, monitor_duration: int):
			...
			logger.info(
				f"Waiting {monitor_duration}s for counter {counter_id} to increment..."
			)
			...
			logger.info(
				f"Counter {counter_id} (monitored for {monitor_duration}s): {final_count}"
			)
			return final_count
